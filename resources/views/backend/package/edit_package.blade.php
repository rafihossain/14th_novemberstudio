...
@section('title', 'Edit Blog package')
@section('content')
<div class="card">
	<div class="card-body">
		<form id="formpackage" action="{{route('backend.update-package')}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group mb-2">
                ...
                <select name="package_category_id" class="form-control">
                    ...
                    @foreach($package_categories as $package_category)
                        ...
                        {{ $package_category->id == $editpackage->package_category_id ? 'selected' : '' }}>{{ $package_category->package_name }}</option>
                    @endforeach
                </select>
			...
			<div class="form-group mb-2">
				...
				<div id="editor" style="height: 300px;">{!! $editpackage->package_description !!}</div>
        ...
        <div class="form-group mb-2">
                <label>Image</label>
                <input type="file" class="imageupload @error('image') is-invalid @enderror" name="package_image"  data-default-file="{{asset($editpackage->image)}}">
                @error('image')
                <strong class="text-danger">{{ $message }}</strong>
                @enderror
            </div>
